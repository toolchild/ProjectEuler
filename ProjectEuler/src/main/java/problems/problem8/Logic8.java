...
public class Logic8 extends LogicBase {
  ...
  public long getResult(Number... numbers) {
    ...
    for (int startIndex = 0; startIndex <= intArrayList.size() - intervallSize; startIndex++) {
      ...
      if (!currentNumbers.contains(0)) {
        long product = getProduct(currentNumbers);
        if (product > result) {
  ...
  private ArrayList<Integer> createIntArrayList(String numbersString) {
    ArrayList<Integer> intArrayList = new ArrayList<Integer>();
    for (int i = 0; i < numbersString.length(); i++) {
      intArrayList.add(Integer.valueOf(numbersString.charAt(i) - 48));
    }
    logConsole.debug(intArrayList);
    return intArrayList;
  }

  private long getProduct(ArrayList<Integer> currentNumbers) {
    long result = 1;
    for (Integer number : currentNumbers) {
      result = result * number;
    }
    logConsole.debug(result);
    return result;
  }
